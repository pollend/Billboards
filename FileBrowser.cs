...
public class FileBrowser {
	...
	protected void ReadDirectoryContents() {
		if (m_currentDirectory == "/") {
			...
			m_currentDirectoryParts = m_currentDirectory.Split(System.IO.Path.DirectorySeparatorChar);
			if (SelectionPattern != null) {
				string[] generation = Directory.GetDirectories(
					System.IO.Path.GetDirectoryName(m_currentDirectory),
					SelectionPattern
				);
		...
		if (BrowserType == FileBrowserType.File || SelectionPattern == null) {
			...
			for (int i = 0; i < m_nonMatchingDirectories.Length; ++i) {
				int lastSeparator = m_nonMatchingDirectories[i].LastIndexOf(System.IO.Path.DirectorySeparatorChar);
		...
		for (int i = 0; i < m_directories.Length; ++i) {
			m_directories[i] = m_directories[i].Substring(m_directories[i].LastIndexOf(System.IO.Path.DirectorySeparatorChar) + 1);
		}
		...
		if (BrowserType == FileBrowserType.Directory || SelectionPattern == null) {
			...
			for (int i = 0; i < m_nonMatchingFiles.Length; ++i) {
				m_nonMatchingFiles[i] = System.IO.Path.GetFileName(m_nonMatchingFiles[i]);
			}
			Array.Sort(m_nonMatchingFiles);
		}
		for (int i = 0; i < m_files.Length; ++i) {
			m_files[i] = System.IO.Path.GetFileName(m_files[i]);
		}
		Array.Sort(m_files);
		BuildContent();
		m_newDirectory = null;
		...
		GUILayout.BeginArea(
				...
				for (int parentIndex = 0; parentIndex < m_currentDirectoryParts.Length; ++parentIndex) {
					if (parentIndex == m_currentDirectoryParts.Length - 1) {
						...
						for (int i = m_currentDirectoryParts.Length - 1; i > parentIndex; --i) {
							parentDirectoryName = System.IO.Path.GetDirectoryName(parentDirectoryName);
						}
			...
			GUILayout.BeginHorizontal();
				...
				if (GUILayout.Button("Select", GUILayout.Width(50))) {
					if (BrowserType == FileBrowserType.File) {
						m_callback(System.IO.Path.Combine(m_currentDirectory, m_files[m_selectedFile]));
					} else {
						if (m_selectedDirectory > -1) {
							m_callback(System.IO.Path.Combine(m_currentDirectory, m_directories[m_selectedDirectory]));
						} else {
	...
	protected void FileDoubleClickCallback(int i) {
		if (BrowserType == FileBrowserType.File) {
			...
			m_callback(System.IO.Path.Combine(m_currentDirectory, m_files[i]));
		}
	}
 
	protected void DirectoryDoubleClickCallback(int i) {
		SetNewDirectory(System.IO.Path.Combine(m_currentDirectory, m_directories[i]));
	}
 
	protected void NonMatchingDirectoryDoubleClickCallback(int i) {
		SetNewDirectory(System.IO.Path.Combine(m_currentDirectory, m_nonMatchingDirectories[i]));
	}
